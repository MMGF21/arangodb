...
V8ContextDealerGuard::V8ContextDealerGuard(v8::Isolate* isolate, TRI_vocbase_t* vocbase, bool allowModification)
  : _context(nullptr)
  , _isolate(isolate)
  , _active(isolate ? false : true)
{
  if (_active) {
    ...
    if (!_context) {
      THROW_ARANGO_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "could not acquire v8 context");
    }
...
v8::Isolate* V8ContextDealerGuard::isolate() {
  return _isolate;
}
